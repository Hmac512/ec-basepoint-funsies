...
class IdentityManager:
    ...
    def generate_child_key(self, id:str):
        r = random_scalar()
        Gr = self.curve.multiply(self.curve.G1, r)
        hsh = sha256(str(Gr).encode("utf-8"))
        hsh.update(id.encode("utf-8"))
        Gr_and_id = int.from_bytes(hsh.digest(), "big") % self.curve.curve_order
...
class IdentityKey:
    ...
    def __init__(self, secret_key, master_public_key, id, Gr) -> None:
        self.secret_key = secret_key
    ...
    def sign(self, msg:str):
        ...
        hsh_scalar = int.from_bytes(hsh.digest(), "big") % self.curve.curve_order


        b = a + ((self.secret_key * hsh_scalar) % self.curve.curve_order)
        return (Ga, b, self.Gr)

    def verify(self, msg:str, Ga, b, Gr):
        hsh = sha256(msg.encode("utf-8"))
        hsh.update(self.id.encode("utf-8"))
        hsh.update(str(Ga).encode("utf-8"))
        ...
        m2 = self.curve.multiply(self.master_public_key, c*d % self.curve.curve_order)
...
if __name__ == "__main__":
    ...
    (usk1,gr1, id1) = ibs.generate_child_key("child-1")
    id_key = IdentityKey(usk1, ibs.public_key_g1, id1, gr1)
