...
class IdentityKey:
    ...
    def sign(self, msg: str):
        ...
        b = a + ((self.secret_key * hsh_scalar) % self.curve.N) % self.curve.N
        return (Ga, b, self.Gr)
...
class IdentityManager:
    ...
    def generate_child_key(self, id: str) -> 'IdentityKey':
        ...
        usk = r + ((self.secret_key * Gr_and_id) % self.curve.N) % self.curve.N
...
if __name__ == "__main__":
    ...
    id_key = ibs.generate_child_key(
        "child-1|max-eth=1|valid-from=<block-num>|valid-until=<block-num>")
